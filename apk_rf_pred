...
def main():
    ...
    st.title("📊 Random Forest Prognoza")
    ...
    if st.button("Uruchom (enhanced)"):
        ...
        try:
            ...
            st.error(f"Błąd modelu: {e}"); return
        ...
        st.subheader("🔍 Feature importances (avg across lags)")
        ...
        st.pyplot(fig)
        # export
        outname = f"{ticker.replace('.','_')}_forecast_enhanced.xlsx"
